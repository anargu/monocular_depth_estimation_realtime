...
class NYUv2(Dataset):
    ...
    def __init__(
        ...
        self.raw_dataset = h5py.File(root, 'r')
        total_dataset =  len(self.raw_dataset['images'])
        train_length = int(0.8 * total_dataset)
        test_length = int(0.1 * total_dataset)
        validation_length = total_dataset - train_length - test_length
        self.raw_images_train, self.raw_images_validation, self.raw_images_test =  torch.utils.data.random_split(self.raw_dataset['images'], (train_length, validation_length, test_length))
        self.raw_depths_train, self.raw_depths_validation, self.raw_depths_test =  torch.utils.data.random_split(self.raw_dataset['depths'], (train_length, validation_length, test_length))
        self.raw_seg_train, self.raw_seg_validation, self.raw_seg_test =  torch.utils.data.random_split(self.raw_dataset['labels'], (train_length, validation_length, test_length))
        # if download:
        #     self.download()
    ...
    def __getitem__(self, index: int):
        ...
        if self.rgb_transform is not None:
            ...
            img = (self.raw_images_train if self._split == 'train' else self.raw_images_test)[index]
            img = np.moveaxis(img, 0, -1)
            img = np.moveaxis(img, 0, 1)
            img = self.rgb_transform(img)
            i,j,h,w = RandomCrop.get_params(img, (400,400))
            img = F.crop(img, i, j, h, w)
            imgs.append(img)

        if self.seg_transform is not None:
            random.seed(seed)
            img = (self.raw_seg_train if self._split == 'train' else self.raw_seg_test)[index]
            img = np.moveaxis(img, 0, -1)
            img = self.seg_transform(img.astype(np.float32))
        ...
        if self.depth_transform is not None:
            ...
            img = (self.raw_depths_train if self._split == 'train' else self.raw_depths_test)[index]
            img = np.moveaxis(img, 0, 1)
    ...
    def __len__(self):
        ...
        # return len(self.raw_dataset['images'])
        return len(self.raw_depths_train if self._split == 'train' else self.raw_depths_test)
